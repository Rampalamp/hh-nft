...
!developmentChains.includes(network.name)
    ...
    : describe("DynamicSvgNft", () => {
          ...
          describe("mintNft", () => {
              it("emits event and creates NFT", async () => {
                  ...
                  //tokenCounter gets incremented before mint process, so starting ID should be 1
                  const tokenURI = await dynamicSvgNft.tokenURI(1);
                  assert.equal(tokenURI, highTokenUri);
              });
              it("shifts the token uri to lower when the pricce is below highValue", async () => {
